...
export default function ChatHistorySidebar({ onSelectChat }) {
  ...
  return (
    <aside className="w-72 bg-slate-900 text-white h-full flex flex-col border-r border-slate-800">
      ...
      <div className="flex-1 overflow-y-auto">
        {chats.length === 0 && <div className="p-4 text-slate-400">No chats yet.</div>}
        {chats.map(chat => (
          <button
            key={chat.chatId}
        ...
        {chat.messages?.[chat.messages.length - 1]?.content?.slice(0, 40) || "New Chat"}
         ...
         <div className="text-xs text-slate-400">{new Date(chat.updatedAt).toLocaleString()}</div>
          </button>
        ))}
      </div>
      <button
